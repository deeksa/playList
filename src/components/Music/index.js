...
class Music extends Component {
  ...
  onTapDeleteButton = id => {
    ...
    const filteredMusicList = musicList.filter(e => e.id !== id)
    this.setState({
  ...
  onChangeInput = event => {
    const {musicList} = this.state
    const filteredInputList = musicList.filter(e =>
      e.name.toLowerCase().includes(event.target.value.toLowerCase()),
    )
    this.setState({
      musicList: filteredInputList,
    })
  }
            ...
            <ul className="list1">
              {musicList.map(e => (
                <ListDisplay
                  key={e.id}
                  each={e}
                  onTapDeleteButton={this.onTapDeleteButton}
                />
              ))}
            </ul>
          )}
